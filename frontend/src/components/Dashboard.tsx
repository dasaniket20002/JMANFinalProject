...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      .get(GET_PERMISSIONS, { params: { role: userData?.role } })
      .then((res) => {
        ...
        <Container containerHeading="Additional Controls" headerType="l2">
          {accessTo.includes("REGISTER_USER") &&
            accessTo?.includes("SEE_USERS") && (
              <CreateUserModule email={userData.email} role={userData.role} />
            )}
...
const CreateUserModule = ({
  ...
  const getAllUsers = () => {
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setUsersArray(res.data.users as User_type[]);
        } else {
