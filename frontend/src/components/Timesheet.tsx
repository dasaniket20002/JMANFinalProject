				...
				<TableBody>
					{timesheetActivities.map((item, index) => (
						<Activity
							...
							increaseActivityRows={increaseActivityRowsForIndex(
								index
							)}
							decreaseActivityRows={decreaseActivityRowsForIndex(
								index
							)}
							...
							setProjectData={setProjectSelectDataForActivity(
								index
							)}
	...
	return (
		<>
			{[...Array(numRows)].map((_, i) => (
				<TableRow key={i}>
					{i === 0 && (
						<TableCell rowSpan={numRows}>{activity_name}</TableCell>
					)}
			...
			{rowData.map((value, index) => (
				<TableCell key={index} className="max-w-12">
					<Input
						...
						value={value}
						onChange={(e) =>
							handleNumericInputs(index, e.target.value)
						}
						disabled={inputsDisabled}
					/>
				</TableCell>
...
const AdditionalControls = ({ userData }: AdditionalControls_type) => {
